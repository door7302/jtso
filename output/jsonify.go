...
func (m *Metadata) UpdateMeta(rd *xml.RawData) error {
	...
	for _, phy := range rd.IfList.Physicals {
		...
		// Keep only WAN ports
		if strings.Contains(phy_name, "et-") || strings.Contains(phy_name, "xe-") || strings.Contains(phy_name, "ge-") || strings.Contains(phy_name, "ae") || strings.Contains(phy_name, "lt-") || strings.Contains(phy_name, "ps-") {
